...
public class GameMessageHandlers {
    ...
    public GamePacket login(ChannelHandlerContext ctx, byte[] payload) throws Exception {
        LoginRequest request = LoginRequest.parseFrom(payload);
        try {
    ...
    public GamePacket heartbeat(ChannelHandlerContext ctx, byte[] payload) throws Exception {
        HeartbeatRequest.parseFrom(payload);
    ...
    public GamePacket chat(ChannelHandlerContext ctx, byte[] payload) throws Exception {
        Long playerId = ctx.channel().attr(SessionManager.PLAYER_ID).get();
        String nickname = ctx.channel().attr(SessionManager.NICKNAME).get();
        if (playerId == null) {
            return null;
        }
        ChatRequest request = ChatRequest.parseFrom(payload);
        ChatMessage message = ChatMessage.newBuilder()
                ...
                .build();
        GamePacket packet = new GamePacket(MsgIds.CHAT_MSG, message.toByteArray());
